...
class Mediumlinescreen extends StatefulWidget {
  const Mediumlinescreen({Key? key}) : super(key: key);

  @override
  _MediumlinescreenState createState() => _MediumlinescreenState();
}

class _MediumlinescreenState extends State<Mediumlinescreen> {
                                  ...
                                  onPressed: () {
                                    if (formKey1.currentState!.validate()) {
                                      formKey1.currentState!.save();
                                       ...
                                       double X = X_calcM(f, L, l);
                                       Complex Y = Y_calcM(f, l, c);
                                       Complex Z = Z_calcM(R, X);

                                       Complex A = A_calcM(Y, Z);
                                       Complex D = A;
                                       Complex B = Z;
                                       Complex C = C_calcM(Y, Z);
                                       ...
                                       if(_value==false){
                                        ...
                                        double MSP=double.parse(complexpower!);
                                         Is=Is_calc_S(MSP, Pf, Vs, _sign1);
                                       }else{
                                        double MI=double.parse(current!);
                                         double Marg=double.parse(argumentofcurrent!);
                                         Is=Complex(MI*cos(Marg),MI*sin(Marg));

                                       }

                                      Complex Vr = Vr_calc_S(A, B, C, Vs, Is);
                                       Complex Ir = Ir_calc_S(A, B, C, Vs, Is);

                                       Complex SP = Complex(3.0) * Vs * Is.conjugate();
                                       Complex RP = Complex(3.0) * Vr * Ir.conjugate();

                                       double effi = effM(SP, RP);
                                       double regu = RegM(Vs, Vr, A);
                                       ...
                                       Complex C = C_calcM(Y, Z);
                                        ...
                                        if(_value2==false){
                                         ...
                                         double MRP=double.parse(complexpowerR!);
                                           Ir = Ir_calcM(MRP, Pf, Vr, _sign2);
                                       }else{
                                         double MI=double.parse(currentR!);
                                         double Marg=double.parse(argumentofcurrentR!);
                                         Ir=Complex(MI*cos(Marg),MI*sin(Marg));
                                       }

                                      Complex Vs = Vs_calcM(A, B, Vr, Ir);
                                      Complex Is = Is_calcM(C, D, Vr, Ir);

                                      Complex SP = Complex(3.0) * Vs * Is.conjugate();
                                      Complex RP = Complex(3.0) * Vr * Ir.conjugate();

                                      double effi = effM(SP, RP);
                                      double regi = RegM(Vs, Vr, A);
